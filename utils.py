...
class V(tuple):
    def __new__(
        cls, x: typing.Union[int, tuple], y: typing.Optional[int] = None
    ) -> "V":  # Create new vector
    ...
    def window(self, width: int, height: int) -> list:  # Get window of coordinates
        return [
            [(i, j) for i in range(self.x, self.x + width)]
            for j in range(self.y, self.y + height)
        ]
    ...
    def neighbors(
        self, coord: tuple, restrict: bool = True
    ) -> list:  # Get all neighbors
        return self.straight_neighbors(coord, restrict) + self.diagonal_neighbors(
            coord, restrict
        )
...
def grid_from_dimensions(
    width: int, height: int, default=None
...
def distance_elements_loop(e1, e2, l):  # Get shortest distance between elements
    standard_distance = abs(l.index(e1) - l.index(e2))
    return min(standard_distance, len(l) - standard_distance)


def shared_prefix(*lists):  # Find the shared prefix of a list of lists.
    if not lists:
        return []
...
def runs(
    list: list, f=lambda a: all([i == a[0] for i in a])
) -> list:  # Split into runs by condition
    l = deepcopy(list)
    result = []
    currun = []
    while l:
